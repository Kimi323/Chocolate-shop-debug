...
//        console.log(display.x);
        // code bellow will make four ChocolateBar objects referencing the
        // same (only one) display object which finally has x = 750.
//		shopStock.addStock(l.name, l.imageFile, l.weight, l.price, l.quantity,
//			display);
        
        // we need to copy display object to a seperate object evey loop
        let copyOfDisplay = {...display};
        shopStock.addStock(l.name, l.imageFile, l.weight, l.price, l.quantity,
			copyOfDisplay);
		display.x += display.width + 35;
        //console.log(copyOfDisplay.x); 
        // finally display.x is 750
	}
...
function ChocolateBar(name, imageFile, weight, price, display) {
	...
	this.draw = function() {
		image(this.image, this.display.x, this.display.y, this.display.width,
			this.display.height);
		var pricePer100g = Number(price / weight * 100).toFixed(2);
